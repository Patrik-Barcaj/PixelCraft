  ...
  return ( // Return JSX
    <div className="min-h-screen bg-brand-black text-white selection:bg-brand-yellow selection:text-black font-sans relative flex flex-col"> {/* Main wrapper */}
      ...
      <motion.header /* Header */
        ...
        className="fixed top-0 left-0 right-0 z-50 px-4 py-4 xl:px-12 xl:py-6 flex flex-nowrap justify-between items-center border-b border-white/5 transition-all duration-300 overflow-hidden" /* Header styling */
      > {/* Close header opening */}
        <Link to="/" className="cursor-pointer group shrink-0"> {/* Logo link */}
          <motion.div /* Motion wrapper for conditional opacity */
            style={{ opacity: location.pathname === '/' ? logoOpacity : 1 }} /* Opacity style */
            className="flex flex-nowrap items-center gap-2 xl:gap-4 shrink-0" /* Flex classes */
          > {/* Close motion div */}
            <img src={logoOnly} alt="Logo" className="h-[26px] md:h-10 w-auto object-contain group-hover:scale-110 transition-transform duration-300 shrink-0" /> {/* Logo image resized for mobile */}
            <img src={pixelCraftText} alt="Pixel Craft" className="h-[26px] md:h-10 w-auto object-contain group-hover:brightness-125 transition-all shrink-0" /> {/* Brand text resized for mobile */}
          </motion.div> {/* Close motion wrapper */}
        </Link> {/* Close logo link */}

        <nav className="hidden xl:flex flex-nowrap justify-end items-center gap-4 xl:gap-6 text-sm font-bold tracking-widest uppercase whitespace-nowrap shrink-0"> {/* Desktop Navigation */}
          ...
          <Link to="/contact" className={`hover:text-black hover:bg-brand-yellow hover:border-brand-yellow transition-all duration-300 bg-white/5 px-4 py-2 xl:px-6 xl:py-3 rounded border border-white/10 shadow-lg ${location.pathname === '/contact' ? 'bg-brand-yellow text-black' : ''}`}>Contact</Link> {/* Contact link */}
        </nav> {/* Close navigation */}

        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="xl:hidden p-2 text-white hover:text-brand-yellow transition-colors z-50 shrink-0"> {/* Hamburger toggle */}
      ...
      <AnimatePresence> {/* Mobile Menu Animation */}
        {isMenuOpen && ( // Render if open
          <motion.div /* Mobile menu overlay */
            ...
            className="fixed inset-0 z-40 bg-black/95 backdrop-blur-2xl flex flex-col pt-24 pb-8 overflow-y-auto xl:hidden" /* Styling */
          > {/* Close motion div */}
            <div className="flex flex-col items-center justify-start gap-5 min-h-max pb-8"> {/* Inner wrapper */}
              <Link onClick={() => setIsMenuOpen(false)} to="/" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase hover:text-brand-yellow transition-colors ${location.pathname === '/' ? 'text-brand-yellow' : 'text-gray-300'}`}>Home</Link> {/* Home link */}
            <Link onClick={() => setIsMenuOpen(false)} to="/about" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase hover:text-brand-yellow transition-colors ${location.pathname === '/about' ? 'text-brand-yellow' : 'text-gray-300'}`}>About</Link> {/* About link */}
            <Link onClick={() => setIsMenuOpen(false)} to="/services" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase hover:text-brand-yellow transition-colors ${location.pathname === '/services' ? 'text-brand-yellow' : 'text-gray-300'}`}>Services</Link> {/* Services link */}
            <Link onClick={() => setIsMenuOpen(false)} to="/pricing" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase hover:text-brand-yellow transition-colors ${location.pathname === '/pricing' ? 'text-brand-yellow' : 'text-gray-300'}`}>Pricing</Link> {/* Pricing link */}
            <Link onClick={() => setIsMenuOpen(false)} to="/portfolio" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase hover:text-brand-yellow transition-colors ${location.pathname === '/portfolio' ? 'text-brand-yellow' : 'text-gray-300'}`}>Portfolio</Link> {/* Portfolio link */}
            <Link onClick={() => setIsMenuOpen(false)} to="/contact" className={`text-xl sm:text-2xl font-bold tracking-widest uppercase text-black bg-brand-yellow px-8 py-3 rounded shadow-lg`}>Contact</Link> {/* Contact link */}
