...
export default function Services() { // Component definition
  return ( // Return JSX
    <section className="w-full max-w-7xl mx-auto px-6 pt-10"> {/* Services container */}
      ...
      > {/* Close motion div */}
        <h1 className="text-4xl md:text-6xl font-bold uppercase tracking-widest mb-6 drop-shadow-lg">Our Services</h1> {/* Section title */}
        <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto font-medium">Bridging the gap between raw vision and flawless execution in game development.</p> {/* Section subtitle */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> {/* Services grid */}
        <MobileCarousel items={[ // Pass cards to carousel
          ...
          <motion.div /* Service 3 card */
            ...
            className="glass-panel p-10 md:p-12 rounded-2xl group hover:border-brand-yellow/60 transition-all duration-500 hover:-translate-y-2 relative overflow-hidden shadow-xl md:col-span-2 lg:col-span-1 bg-black/60 cursor-pointer w-full h-full" /* Card styling */
          > {/* Close motion div */}
            <div className="absolute top-0 right-0 w-40 h-40 bg-brand-yellow/5 rounded-bl-full -z-10 group-hover:bg-brand-yellow/10 transition-colors duration-500 blur-xl" /> {/* Decorative corner glow */}
